...
export class MemStorage implements IStorage {
  ...
  constructor() {
    ...
    // Create admin user with a properly formatted password
    // Since we can't hash it synchronously, we'll use a pre-hashed password
    // This represents "admin123" hashed with scrypt
    const hashedPassword = "5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8.abcdef123456";
  ...
  private createSamplePosts(authorId: number) {
    const samplePosts = [
      ...
      },
      {
        ...
        excerpt:
          "The technical interview landscape is constantly evolving. Learn the latest strategies, common questions, and preparation techniques to ace your next interview.",
        content:
        ...
        category: "Career Development",
        featuredImage:
          "https://images.unsplash.com/photo-1596496181871-9681eacf9764?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1050&q=80",
        status: "published",
        readTime: 6,
      },
      {
      ...
      },
      {
        ...
        featuredImage:
          "https://images.unsplash.com/photo-1591453089816-0fbb971b454c?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1050&q=80",
        status: "published",
        readTime: 7,
      },
    ];
  ...
  async createSubscriber(
    subscriberData: InsertSubscriber,
  ): Promise<Subscriber> {
  ...
  try {
    ...
    if (global.useMemoryStorage) {
      ...
      try {
        ...
        console.error(
          "Error creating MongoDB storage, falling back to memory storage",
          storageError,
        );
        global.useMemoryStorage = true;
        return new MemStorage();
    ...
    console.error(
      "Error creating storage, falling back to memory storage",
      error,
    );
    global.useMemoryStorage = true;
    return new MemStorage();
